      ...
      <div className="px-5 pt-6">
        ...
        <motion.div variants={container} initial="hidden" animate="show">
          ...
          <motion.div variants={item} className="grid grid-cols-3 gap-3 mb-6">
            <StatCard icon={<Users size={18} />} label="Jugadores" value={players.length} color="text-blue-400" bg="bg-blue-500/10" />
            <StatCard icon={<Trophy size={18} />} label="Partidos" value={matches.length} color="text-yellow-400" bg="bg-yellow-500/10" />
            <StatCard icon={<TrendingUp size={18} />} label="Esta semana" value={1} color="text-green-400" bg="bg-green-500/10" />
          </motion.div>
...
function StatCard({ icon, label, value, color, bg }: { icon: React.ReactNode; label: string; value: number; color: string; bg: string }) {
  return (
    <div className={`p-3 rounded-2xl ${bg} flex flex-col gap-2`}>
